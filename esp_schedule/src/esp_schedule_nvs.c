...
static esp_schedule_handle_t esp_schedule_migrate_from_v1(const char *nvs_key, const uint8_t *blob, size_t buf_size)
{
    ...
    esp_schedule_trigger_t *triggers = (esp_schedule_trigger_t *)ESP_SCHEDULE_MALLOC(sizeof(esp_schedule_trigger_t));
    if (triggers == NULL) {
        ESP_SCHEDULE_LOGE(TAG, "Could not allocate trigger while migrating %s", nvs_key);
...
static esp_schedule_handle_t esp_schedule_nvs_get(const char *nvs_key)
{
    ...
    if (persistent->trigger_size != (uint16_t) sizeof(esp_schedule_trigger_t)) {
        ESP_SCHEDULE_LOGE(TAG, "Schedule %s trigger size %u != %u; rejecting", nvs_key, persistent->trigger_size, (unsigned)sizeof(esp_schedule_trigger_t));
        ESP_SCHEDULE_FREE(blob_buffer);
        return NULL;
    }
    ...
    if (buf_size < schedule_size + trigger_list_size) {
        ESP_SCHEDULE_LOGE(TAG, "Schedule %s blob truncated (%u < header+%u triggers); rejecting", nvs_key, (unsigned)buf_size, persistent->trigger_count);
        ESP_SCHEDULE_FREE(blob_buffer);
        return NULL;
    }
