...
static void create_example_schedules(void)
{
    ...
    esp_schedule_handle_t multi_handle = esp_schedule_create(&multi_schedule);
    if (multi_handle) {
        ESP_LOGI(TAG, "Created multi-trigger schedule successfully");
        esp_schedule_enable(multi_handle);
    }
...
void app_main(void)
{
    ...
    esp_schedule_handle_t *schedule_list = esp_schedule_init(true, NULL, &schedule_count, NULL);
