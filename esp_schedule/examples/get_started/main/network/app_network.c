...
#ifdef CONFIG_ESP_SCHEDULE_EXAMPLE_PROV_POP
static const char *s_pop = CONFIG_ESP_SCHEDULE_EXAMPLE_PROV_POP;
#else
static const char *s_pop = "12345678"; // Default PoP value
#endif // CONFIG_ESP_SCHEDULE_EXAMPLE_PROV_POP
...
static void display_qr_code(const char *service_name, const char *transport)
{
    ...
    if (s_pop && strlen(s_pop) > 0) {
        snprintf(payload, sizeof(payload), "{\"ver\":\"v1\",\"name\":\"%s\",\"pop\":\"%s\",\"transport\":\"%s\"}",
                service_name, s_pop, transport);
    } else {
        snprintf(payload, sizeof(payload), "{\"ver\":\"v1\",\"name\":\"%s\",\"transport\":\"%s\"}",
...
#if defined(CONFIG_ESP_SCHEDULE_EXAMPLE_PROV_BLE)
        display_qr_code(service_name, "ble");
#elif defined(CONFIG_ESP_SCHEDULE_EXAMPLE_PROV_SOFTAP)
        display_qr_code(service_name, "softap");
        /* For SoftAP, create WiFi AP interface */
        esp_netif_create_default_wifi_ap();
#endif

        /* Start provisioning service */
        ret = network_prov_mgr_start_provisioning(NETWORK_PROV_SECURITY_1, s_pop, service_name, NULL);
        if (ret != ESP_OK) {
